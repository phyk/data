...
tryCatch(load(rdaPathClick),
         error=function(e){
           didNotImport <<- TRUE
           print("konnte click nicht importieren")
         })
tryCatch(load(rdaPathOrder),
         error=function(e){
           didNotImport <<- TRUE
           print("konnte order nicht importieren")
         })
```
...
Most of the customers are located in California (`r order_map$x[5]`) and New York (`r order_map$x[35]`).<br><br>
Yet, all insights refer to the order dataset. The clickstream dataset provides a lot more data but not necessarily more interesting insights. As the name implicates, it carries information about where the customer clicked on the website. 
...
# count_2 <- count(cl, Request_Week == "2000-02-28")[2,2]
# temp <- count_2 / count(or)
# count_2_p <- percent(temp[1,1])
```

What can be observed is, that there is one day, April 27, with far more than average requests (`r count_3`). The amount of requests in the morning is far above average on this day.
...
The table "Summary_Order" is based on `r nrow(ave_Order)` order observations. We extracted these orders from the order dataset.The negative minimum in the row b Order Amountb  looks suspicious. Research in the data revealed that the orders: order-Id 12314 and order-Id 10930 used a special combination of products and the promotion code b FRIENDb  to generate a negative order amount.  However, the credit card payment amount indicates that the coups didnb t succeed.  Also the datasets cannot tell if the negative Order Amount were create willingly or through accident. 
...
=======
>>>>>>> 043a2461f48a7329e3a5ae111acd81fcdfc9b3af
## Predictions
